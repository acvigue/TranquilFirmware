...
TrinamicsController::~TrinamicsController() { deinit(); }

void TrinamicsController::deinit() {
    // Release pins
    if (_tx1 >= 0) pinMode(_tx1, INPUT);
    if (_tx2 >= 0) pinMode(_tx2, INPUT);
    _tx1 = _tx2 = -1;
...
void TrinamicsController::configure(const char* configJSON) {
    ...
    if ((mcChip == "TMC2208" || mcChip == "TMC2209")) {
        ...
        if (TX1Pin == -1 || TX2Pin == -1) {
            Log.warning("%ssetup UART pins invalid\n", MODULE_PREFIX);
        } else {
            _tx1 = TX1Pin;
            _tx2 = TX2Pin;
            _isEnabled = true;
...
uint32_t TrinamicsController::getUint32WithBaseFromConfig(const char* dataPath, uint32_t defaultValue, const char* pSourceStr) {
    String confStr = RdJson::getString(dataPath, "", pSourceStr);
    if (confStr.length() == 0) return defaultValue;
